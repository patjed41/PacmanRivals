...
int main() {
    ...
    sf::Vector2i lin_position = lin_ghost.test_getDownTile();
    sf::Vector2i lin_expected = {1, 2};

    TestCycleGhost cyc_ghost = TestCycleGhost(map, 3, 3, {1, 3, 0, 2});
    sf::Vector2i cyc_position = cyc_ghost.test_getDownTile();
    sf::Vector2i cyc_expected = {3, 4};

    TestRandomGhost ran_ghost = TestRandomGhost(map, 5, 5, 3);
    sf::Vector2i ran_position = ran_ghost.test_getDownTile();
